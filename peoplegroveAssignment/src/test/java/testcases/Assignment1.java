...
public class Assignment1 extends CommonUtil {
	...
	public void AskQuestionCheck() throws InterruptedException {
		...
		try {
			List<WebElement> joblinks = checkElementSize("jobLinks_XPATH"); // counting the size of job links
	...
	@Test(priority = 3, dependsOnMethods = "AskQuestionCheck", dataProviderClass = DataUtil.class, dataProvider = "jobPage", groups = {
			"Assignment" })
	...
	@Test(priority = 4, dependsOnMethods = "JobsPageCheck", dataProviderClass = DataUtil.class, dataProvider = "inboxPage", groups = {
			"Assignment" })
	...
	public void RecrtuierPageCheck() {
		try {
			scroll("otherSimilars_XPATH", 0, 0);
			System.out.println("Scrolled to Other Similar Options!!");

			List<WebElement> otherSimilar = checkElementSize("similarNames_XPATH");
